                ...
                <div className="bg-zinc-950 rounded-[2.8rem] overflow-hidden h-[720px] w-full relative flex flex-col border border-zinc-800">
                  ...
                  <div className="bg-emerald-600 px-6 pt-12 pb-6 text-white flex-shrink-0 relative">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">MyBud Live</span>
                      <div className="w-2 h-2 rounded-full bg-emerald-300 animate-pulse" />
                    </div>
                    <h4 className="text-xl font-black">{brandName} Lab</h4>
                    <p className="text-sm font-bold text-emerald-100">Vegetative Stage — Week 3</p>
                  </div>
...
const TasksMockup: FC<MockupProps> = ({ brandName }) => (
  <div className="space-y-4">
    <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-6">Próximas Tarefas</p>
    ...
    <MockItemWrapper>
      <div className="flex gap-4 opacity-50">
        ...
        <div>
          <h4 className="text-zinc-400 font-bold">Verificar pH do solo</h4>
          <p className="text-xs text-zinc-600 mt-1">Concluído há 2h</p>
        </div>
      </div>
    </MockItemWrapper>
  </div>
  ...
  <div className="space-y-6">
    {[1, 2, 3].map(i => (
      <div key={i} className="flex gap-6 relative">
        ...
        <div className="flex-1">
          <h5 className="text-xs font-black text-zinc-500 mb-2 uppercase">Dia 2{4 - i}</h5>
          <div className="p-4 bg-zinc-900 border border-zinc-800 rounded-2xl">
            <p className="text-sm font-bold text-white">Fertilização Vegetativa</p>
          </div>
        </div>
      </div>
    ))}
  ...
  <div className="space-y-6">
    <div className="bg-zinc-900/80 rounded-3xl p-6 border border-zinc-800">
      <div className="flex justify-between items-center mb-8">
        <span className="text-lg font-black text-white">Nova Rega</span>
        <Droplet className="text-emerald-500" />
      </div>
      <div className="space-y-4">
        <div className="flex justify-between p-4 bg-zinc-950 rounded-2xl border border-zinc-800">
          <span className="text-zinc-500 font-bold uppercase text-[10px]">Volume</span>
          <span className="text-white font-black">2500 ml</span>
        </div>
        <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl">
          <p className="text-[10px] font-black text-emerald-500 uppercase mb-3">Produto</p>
...
const RecommendationsMockup: FC<MockupProps> = ({ brandName }) => (
  <div className="space-y-6">
    <div className="p-6 bg-emerald-500 rounded-[32px] text-zinc-950 relative overflow-hidden">
      ...
      <h4 className="text-xl font-black mb-2 tracking-tight">Dica do Especialista</h4>
      <p className="text-sm font-bold leading-relaxed">
        O estágio de floração exige mais potássio. Recomendamos aumentar a dose de {brandName} para 4ml/L a partir da próxima semana.
      </p>
    </div>
  </div>
);
...
const BrandHubMockup: FC<MockupProps> = ({ brandName }) => (
  <div className="space-y-6">
    ...
    <div className="pt-8 px-2">
      ...
      <p className="text-sm text-zinc-500 font-medium mt-2">Nutrição avançada para colheitas extraordinárias.</p>
      <button className="w-full mt-8 py-4 bg-emerald-500 text-zinc-950 font-black rounded-2xl">Seguir Marca</button>
    </div>
  </div>
);
...
const CalculatorMockup: FC<MockupProps> = ({ brandName }) => (
  <div className="space-y-6">
    <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-3xl">
      <h4 className="text-lg font-black text-white mb-6">Calculadora de Nutrientes</h4>
      <div className="space-y-4">
        <div>
          <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block mb-2">Tamanho do Tanque</label>
  ...
  <div className="space-y-6">
    <div className="aspect-[3/4] bg-zinc-900 rounded-[32px] border-2 border-dashed border-zinc-800 flex items-center justify-center p-8 text-center relative overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
        ...
        <p className="text-sm font-bold text-zinc-500">Stickers de Comunidade em breve</p>
      </div>
      {/* Fake Sticker */}
      <div className="absolute bottom-10 right-6 bg-white p-3 rounded-2xl shadow-2xl rotate-6 flex flex-col items-center border-[4px] border-white active:scale-110 transition-transform cursor-pointer">
        ...
        <span className="text-[8px] font-black text-emerald-800 uppercase">Powered by</span>
